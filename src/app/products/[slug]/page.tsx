...
const mostClicked = await getMostClickedProducts(6);
const moreProducts = await getMoreProducts(product.id, 12);
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-3 pt-2">
            <a
              ...
              className={cn(
                "flex items-center justify-center gap-3 w-full py-4 px-6 rounded-xl text-white font-bold text-lg transition-all active:scale-95 shadow-lg",
                storeColor
              )}
            >
            ...
            <p className="text-xs text-center text-gray-400">
              You will be redirected to {product.affiliate_store}. *Affiliate
              Link
            </p>
          </div>
        </div>
        ...
        {product.tags && product.tags.length > 0 && (
          <div className="mt-5 flex flex-wrap gap-2">
            {product.tags.map((tag) => (
              <span
                key={tag}
                className="badge bg-gray-100 text-gray-600 px-3 py-1 text-sm"
              >
                #{tag}
              </span>
            ))}
